...
class TSpamReview_Plugin implements Typecho_Plugin_Interface
{
	...
	public static function onBeforeComment($comment, $post = null, $widget = null)
	{
		try {
			...
			$originalStatus = isset($comment['status']) ? (string)$comment['status'] : '';
			self::debug('[hook] original status=' . ($originalStatus !== '' ? $originalStatus : 'n/a'));
		...
		if ($blockGarbledAuthor && self::hasGarbledContent($commentAuthor, $commentMail, $commentText)) {
			...
			// 4) 中文检测：评论内容
			$contentAction = isset($opts->contentChineseAction) ? (string)$opts->contentChineseAction : 'A';
			$commentTextHasCn = self::stringHasChinese(isset($comment['text']) ? (string)$comment['text'] : '');
			if (!$commentTextHasCn) {
				if ($contentAction === 'C') {
					...
					if (self::isHoldEligible($comment, $originalStatus)) {
						self::debug('[hold] content no Chinese → set status to waiting');
						$comment['status'] = 'waiting';
					} else {
						self::debug('[hold skip] content no Chinese but status already approved or whitelisted');
					}
				}
			}
			...
			// 5) 中文检测：昵称
			$authorAction = isset($opts->authorChineseAction) ? (string)$opts->authorChineseAction : 'A';
			$authorHasCn = self::stringHasChinese(isset($comment['author']) ? (string)$comment['author'] : '');
			if (!$authorHasCn) {
				if ($authorAction === 'C') {
					...
					if (self::isHoldEligible($comment, $originalStatus)) {
						...
						self::debug('[hold skip] author no Chinese but status already approved or whitelisted');
					}
				}
			}
			...
			if ($baiduEnabled) {
				...
				if ($apiKey !== '' && $secretKey !== '') {
				self::debug('[baidu] ready to audit; comment status=' . (isset($comment['status']) ? (string)$comment['status'] : 'n/a'));
	...
	private static function isHoldEligible($comment, $originalStatus)
	{
		...
		try {
			...
			$needModeration = !empty($options->commentsRequireModeration) || $originalStatus === 'waiting';
			if ($author !== '' && $mail !== '') {
				$db = Typecho_Db::get();
				$prefix = $db->getPrefix();
				$hasApprovedHistory = (int)$db->fetchObject(
			...
			self::debug('[hold] eligibility check error: ' . $e->getMessage());
		}
	...
	public static function onFinishComment()
	{
		...
		if ($skipAdmin && self::isAdmin()) {
			...
			if ($willDeny && $coid > 0) {
				try {
					self::logBlockedComment(['coid' => $coid, 'author' => $author, 'mail' => $mail, 'text' => $text], '后置删除');
